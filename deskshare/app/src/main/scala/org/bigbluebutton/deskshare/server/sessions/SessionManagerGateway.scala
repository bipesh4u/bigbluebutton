...
class SessionManagerGateway(actorSystem: ActorSystem, streamManager: ActorRef, keyFrameInterval: Int,
  ...
  val sessionManager = new SessionManagerSVC(actorSystem, streamManager, keyFrameInterval,
    interframeInterval, waitForAllBlocks).actorRef
