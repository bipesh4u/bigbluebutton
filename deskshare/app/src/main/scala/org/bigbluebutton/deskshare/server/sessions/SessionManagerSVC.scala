...
import akka.actor._
import org.bigbluebutton.deskshare.server.sessions.SessionManagerSVC._
...
object SessionManagerSVC {
  def props(actorSystem: ActorSystem, streamManager: ActorRef, keyFrameInterval: Int,
    interframeInterval: Int, waitForAllBlocks: Boolean): Props = Props(classOf[StreamManager],
    actorSystem, streamManager, keyFrameInterval, interframeInterval, waitForAllBlocks)
...
class SessionManagerSVC(actorSystem: ActorSystem, streamManager: ActorRef, keyFrameInterval: Int, interframeInterval: Int,
    waitForAllBlocks: Boolean) extends Actor with ActorLogging {

  val actorRef = context.actorOf(SessionManagerSVC.props(actorSystem, streamManager, keyFrameInterval,
    interframeInterval, waitForAllBlocks), "session-manager-actor")
  ...
  def receive = {
    case msg: CreateSession =>
      createSession(msg); printMailbox("CreateSession")
    case msg: RemoveSession =>
      removeSession(msg.room); printMailbox("RemoveSession")
    case msg: SendKeyFrame =>
      sendKeyFrame(msg.room); printMailbox("SendKeyFrame")
  ...
  private def createSession(c: CreateSession): Unit = {
    ...
    sessions.get(c.room) match {
      case None => {
        ...
        val session: SessionSVC = new SessionSVC(actorSystem, this, c.room, c.screenDim, c.blockDim,
          streamManager, keyFrameInterval, interframeInterval, waitForAllBlocks, c.useSVC2)

        if (session.initMe()) {
          val old: Int = sessions.size
          sessions += c.room -> session
          //				  session.start
          session.actorRef ! StartSession
  ...
  private def updateMouseLocation(room: String, mouseLoc: Point, seqNum: Int): Unit = {
    sessions.get(room) match {
      case Some(s) =>
        s.actorRef ! new UpdateSessionMouseLocation(mouseLoc, seqNum)
  ...
  private def updateBlock(room: String, position: Int, blockData: Array[Byte], keyframe: Boolean, seqNum: Int): Unit = {
    sessions.get(room) match {
      case Some(s) =>
        s.actorRef ! new UpdateSessionBlock(position, blockData, keyframe, seqNum)
